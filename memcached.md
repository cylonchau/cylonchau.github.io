...
Memcached服务分为服务端和客户端两部分，其中，<font style="background:#ffc104;" size=2>服务端</font>软件的名字形如Memcached-1.4.24.tat.gz，<font style="background:#ffc104;" size=2>客户端</font>软件的名字形如Memcache-2.25.tar.gz

Memcached软件诞生于2003年，最初由LiveJournal的BradFitzpatrick开发完成。Memcached是整个项目的名称，而Memcached是服务器端的主程序名，因其协议简单，使用部署方便、且支持高并发而被互联网企业广泛使用，知道现在仍然被广泛应用。官方网址：http:/../../../images/memcached.org
...
| Redis/Mongodb../../../images/memcachedb | 可持久化存储，即使用内存也会使用磁盘存储 | 1. 缓存后端数据库的查询数据<br>2.作为关系数据库的重要补充 | 1.作为缓存：主要缓存用户重复请求的动态内容：例如BLOG的博文、BBS的帖子等内容。</br>2.作为数据库的有效补充：例如：好友关注、粉丝统计、业务统计等功能可以用持久化存储。 |
...
```sh
./configure --prefix=/app../../../images/memcached
make && make install
```
...
```sh
./configure \
--with-memcached=/usr/local../../../images/memcached \
...
```conf
extension=/app/php-5.5/lib/php/extensions/no-debug-non-zts-20121212../../../images/memcached.so
```
...
```sh
$ /home../../../images/memcached/bin../../../images/memcached -m 16m -p 11211 -d -u root -c 8192
```
...
tcp        0      0  0.0.0.0:11211     0.0.0.0:*         LISTEN      16729../../../images/memcached           
tcp        0      0 0.0.0.0:11212      0.0.0.0:*         LISTEN      16741../../../images/memcached                
tcp6       0      0 :::11211           :::*              LISTEN      16729../../../images/memcached     
tcp6       0      0 :::11212           :::*              LISTEN      16741../../../images/memcached              
udp        0      0 0.0.0.0:11211      0.0.0.0:*                     16729../../../images/memcached     
udp        0      0 0.0.0.0:11212      0.0.0.0:*                     16741../../../images/memcached     
udp6       0      0 :::11211           :::*                          16729../../../images/memcached     
udp6       0      0 :::11212           :::*                          16741../../../images/memcached 
```
...
```sh
memcached -m 16m -p 11211 -d -u root -c 8192 -P /var/run../../../images/memcached/11211.pid
memcached -m 16m -p 11211 -d -u root -c 8192 -P /var/run../../../images/memcached/11212.pid
```

此时，即可以通过kill命令关闭Memcached

```sh
kill `cat /var/run../../../images/memcached/11211.pid`
```
> **关闭Memcached的方法小结如下：**
